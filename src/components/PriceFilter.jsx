...
import {setPrice} from '../stores/slices/filters/priceFilterSlice'
function PriceFilter() {
    const [minPrice,setMinPrice]= useState(400);
    const dispatch = useDispatch()
  return (
    <div className='space-y-4 my-3 flex flex-col justify-center px-4'>
        ...
        <div className='flex justify-start items-center align-middle gap-6 ml-5'>
            ...
            <div className="relative  text-center">
                ...
                <input 
                    ...
                    value={minPrice} 
                    className="w-20 border font-semibold border-gray-300 focus:outline-none focus:ring-1 focus:ring-gray-200 text-gray-500 text-sm rounded-lg block pl-5 p-1.5" />
            </div>
        </div>
        <div>
        <button type="button" onClick={()=>dispatch(setPrice(minPrice))} className="text-white bg-green-600 hover:bg-green-700 focus:outline-none rounded-lg md:w-48 text-sm px-6 py-1.5 text-center mr-2 mb-2 dark:bg-green-500 dark:hover:bg-green-700 dark:focus:ring-green-800">Set Price</button>
        </div>
    </div>
  )
}
